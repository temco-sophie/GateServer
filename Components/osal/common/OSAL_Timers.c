...
#if defined ( CC2530_TIME_SYNC_ROUTER) || defined ( CC2531_TIME_SYNC_COORD)
/*********************************************************************
 * @fn      osal_ChangeSystemClock()
 *
 * @brief   Read the local system clock.
 *
 * @param   Time correction
 *
 * @return  none
 */
void osal_ChangeSystemClock(int32 time_correction)
{
  uint8 timeAlive[4];
  uint32 tmp32;
  ...
  {
    if ( osal_systemClock > ( -time_correction))
      osal_systemClock += time_correction;
    else if ( osal_systemClock < ( -time_correction))
      osal_systemClock = -( time_correction - osal_systemClock);      
  }
  else
    osal_systemClock += time_correction;
//  osal_systemClock &= ~BV(31);
  osal_systemClock &= 0x7FFFFFFF;
//   MicroWait(30);  // for test LJ
  tmp32 = osal_systemClock;
  ...
  MT_BuildAndSendZToolResponse( 0x67, 0x12, sizeof(timeAlive), timeAlive);
}
/*********************************************************************
 * @fn      osal_ChangeSystemClock()
 */
void osal_SetSystemClock( uint32 timeValue)
{
  osal_systemClock = timeValue;
  MT_BuildAndSendZToolResponse( 0x67, 0x12, sizeof(uint32), ( uint8 *)&osal_systemClock);
}
#endif
/*********************************************************************
*********************************************************************/
